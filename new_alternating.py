...
class AlternatingMinimization:
    def __init__(self, T: int, t_s: int, m: int, v_list: list[npt.NDArray], num_joints: int=10, alpha: float=0.001, num_nonzero: Optional[int]=None, seed: Optional[int]=None) -> None:
    ...
    def solve_S(self) -> npt.NDArray:
        ...
        for i in range(self.m):
            ...
            # build one grasp at a time
            A_j_single = np.zeros((self.num_joints * self.T, self.num_joints * self.t_s))
            ...
            for shift in range(self.max_shift):
                ...
                # reconstruct D_jk
                D_jk = np.zeros((self.num_joints * self.T, self.num_joints * self.t_s))
                for joint in range(self.num_joints):
                    for t_s_index in range(self.t_s):
                        t = shift + t_s_index
                        """
                        if t >= self.T:
                            continue
                        """
                        row = joint * self.T + t
                        col = joint * self.t_s + t_s_index
                        D_jk[row, col] = 1
                
                A_j_single += coeff * D_jk
                
            A_j = np.vstack([A_j_single for _ in range(self.num_grasps)]) 
            
            s_flat, *_ = np.linalg.lstsq(A_j, r_j, rcond=None)
...
if __name__ == '__main__':
    ...
    print(f"c length {len(test.c)}")
    print(f"c_true: {np.count_nonzero(test.c_true)}")
